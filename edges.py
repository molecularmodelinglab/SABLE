...
from nodes.connect_external_library import connect_external_library_node
...
from nodes.record_to_json import record_to_json_node

# Create the graph
molecular_graph = StateGraph(dict)  # You'll need to define your state schema
...
molecular_graph.add_node("connect_external_library", connect_external_library_node)
...
def route_molecule_source(state):
    ...
    if tracker.molecule_source and tracker.molecule_source.value == "screening_library":
        return "external_library"
    elif tracker.starting_molecules:  # Has starting molecules to validate
...
molecular_graph.add_edge("generate_molecule", "decide_library")  # ADDED - Missing connection
molecular_graph.add_edge("enumerate_molecules", "bo_iteration")
molecular_graph.add_edge("connect_external_library", "bo_iteration")
...
molecular_graph.add_edge("summarize_results", "record_to_json")  # ADDED - Missing connection
molecular_graph.add_edge("record_to_json", END)

# Remove external_library from setup routing
molecular_graph.add_conditional_edges(
    ...
    route_molecule_source,
    {
        ...
        # Remove: "external_library": "connect_external_library"
    }
)
...
molecular_graph.add_conditional_edges(
    ...
    route_library_strategy,
    {
        ...
        "external_library": "connect_external_library"
    }
)
