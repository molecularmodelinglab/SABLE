...
from tools.molecule_characterization_tool import MoleculeCharacterizationTool, MoleculeCharacterizationInput
...
def run_bo_enumeration_workflow(
    ...
    enumeration_sim_threshold: float = 0.1,
    ...
    print(f"Enumerated {len(memory['enumerated_df'])} candidate molecules.")
    memory['enumerated_df'].to_csv( "scratch/bo_enumeration_table.csv", index=False)
    
    
    # print(memory)
    # search_space_smiles_dict = {str(i): smi for i, smi in enumerate(enumerated_smiles)}
    memory['enumerated_molecules'] = {f"mol_{i}": smi for i, smi in enumerate(memory['enumerated_df'])}
    print(memory['enumerated_molecules'])

    targets_config = []
    for i, prop_name in enumerate(target_property):
        bounds = target_bounds[i] if target_bounds else None
        targets_config.append({"name": prop_name, "mode": target_mode[i], "weight": target_weight[i], "bounds": bounds, "transformation": target_transformations})
    ...
    for i in range(n_iterations):
        ...
        print(memory['bo_recommendations'])

        newly_evaluated_count = 0
        for mol_id_str in memory['bo_recommendations']:
            if mol_id_str in evaluated_ids:
                print(f"Skipping already evaluated molecule ID: {mol_id_str}")
                continue

            smiles_to_evaluate = memory["enumerated_molecules"].get(mol_id_str)
            ...
            try:
                properties = char_tool._run(memory=memory)

                target_values = {}
                all_targets_found = True
                for prop_name in target_property:
                    value = memory['characterization_results'][mol_id_str][prop_name]
                    if value is None:
                ...
                if memory['characterization_results']:
                    measurement = {
                        "iteration": i + 1,
    ...
    results_df.to_csv("scratch/bo_enumeration_results.csv", index=False)
    return results_df


if __name__ == "__main__":
    start_smiles = "CCC1C(=O)N(C)C(C(C)C)C(=O)NC(C(=O)NC(C(=O)N(C)C(C(C)C)C(=O)NC(CC(C)C)C(=O)N(C)C(CC(C)C)C(=O)NC(C(C)O)C(=O)N(C)C(CC(C)C)C(=O)NC(C)C(=O)N(C)C(CC=C)C(=O)N1)C(C)CC)C(C)CC"
    targets = ["QED"]
    modes = ["MAX"]
    weights = [0.6]
    # bounds = [(0,1)]
