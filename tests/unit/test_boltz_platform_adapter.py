...
@pytest.mark.asyncio
async def test_platform_adapter_rejects_missing_protein_sequence():
    adapter = BoltzPlatformAdapter("secret", client_factory=lambda **kwargs: None)
    request = platform_request().model_copy(update={"proteins": [{"uniprot_id": "P12345"}]})

    with pytest.raises(ValueError, match="require protein sequences"):
